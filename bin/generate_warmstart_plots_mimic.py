...
dbs_paths = list(Path("results/warmstart_dbs/mimic").iterdir())
plot_dfs = list(map(generate_plot_df, tqdm(dbs_paths)))
for i, d in enumerate(plot_dfs):
    d["fold"] = i
df = pd.concat(plot_dfs, axis=0)
df.to_csv("results/warmstart_plots/mimic_df.csv", index=False)
df = pd.read_csv("results/warmstart_plots/mimic_df.csv")
...
plt.savefig("results/warmstart_plots/mimic_best_counts.png", bbox_inches="tight")
plt.clf()
...
fig, ax = plt.subplots(ncols=4, nrows=1, figsize=(25, 4.5), constrained_layout=False, sharey=True)
...
for i, model in enumerate(models):
    for j, fold in enumerate(fold_ids):
        ...
        if i == 0 and j == len(fold_ids) - 1:
            p.legend(loc=(-1.75, 1.15), ncol=4, frameon=False)
            plt.setp(p.get_legend().get_title(), fontsize=22)
            plt.setp(p.get_legend().get_texts(), fontsize=22)
        else:
            p.legend([], [], frameon=False)
plt.savefig("results/warmstart_plots/mimic_adtm.png", bbox_inches="tight")
plt.clf()
...
for model in models:
    for it in iterations:
        ...
        draw_cd_diagram(
            f"results/warmstart_plots/mimic_{model}_{it + 1}.png",
            df_perf=df_to_cd,
            title=f"MIMIC - {model} - it. {it + 1}",
            labels=False,
        )
